...
<textarea id="wpl_c_<?php echo $field->id; ?>" rows="<?php echo $options['rows']; ?>" cols="<?php echo $options['cols']; ?>" onchange="metatag_key_creator();" <?php echo ($options['readonly'] == 1 ? 'disabled="disabled"' : ''); ?>><?php echo $value; ?></textarea>
...
function metatag_key_creator()
{
	...
	try
	{
		if(wplj.trim(wplj("#wpl_c_55").val()) != '0' && wplj.trim(wplj("#wpl_c_55").val()) != '')
			start += number_to_th(wplj("#wpl_c_55").val()) + " <?php echo __('Floor', WPL_TEXTDOMAIN); ?>, ";
	}
	catch(err)  { }
	...
	try
	{
		if(wplj.trim(wplj("#wpl_c_9").val()) != '0' && wplj.trim(wplj("#wpl_c_9").val()) != '')
			start += wplj("#wpl_c_9").val() + " <?php echo __('Bathrooms', WPL_TEXTDOMAIN); ?>, ";
	}
	catch(err)  { }
	...
	try
	{
		if(wplj.trim(wplj("#wpl_c_5").val()) != '0' || wplj.trim(wplj("#wpl_c_5").val()) != '-1')
			start += "<?php echo __('Listing ID', WPL_TEXTDOMAIN); ?> "+wplj("#wpl_c_5").val();
	}
	catch(err)  { }
