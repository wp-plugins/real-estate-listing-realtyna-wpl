...
(function ($, window, document, undefined) {
    ...
    //region + PrettyPhoto JS & Libo funtion
    // TODO Have to replace with Realtyna Lightbox
    if (!$.fancybox)
        $.fancybox = {};
    ...
    rta.internal = (function () {
        return {
            ...
            initChosen: function () {
                $("select[data-has-chosen],.prow select, .panel-body > select, .fanc-row > select, .fanc-content-body select").not('[data-chosen-opt],[data-chosen-disable]').chosen(rta.config.chosen);
    ...
    realtyna.tabs = function () {
        ...
        $(_tabOptions.tabSystemClass).each(function(){
            var _tabs = $(this).find('>' + _tabOptions.tabsClass),
                _tabContents = $(this).find('>'+ _tabOptions.tabContentsClass);
...
wplj(function(){
    ...
    wplj._realtyna.lightbox.on('afterOpened',function(){
        rta.internal.initChosen();
    });
