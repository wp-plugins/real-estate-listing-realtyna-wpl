...
function wpl_listing_location_change(location_level, value)
{
	...
	{
		if (wplj("#wpl_listing_location_level_containerzips").length)
		{
			/** remove form element and reset current data **/
			wplj("#wpl_listing_location_level_containerzips").remove();
	...
	{
		if (!(wplj("#wpl_listing_location_level_container" + i).length > 0))
			continue;
		if (i >= 3 && location_method == '1')
			break;

		/** remove form element and reset current data **/
		wplj("#wpl_listing_location_level_container" + i).remove();
	...
	/** load next level **/
	wpl_load_location_select(next_level, value);
...
function wpl_load_location_select(location_level, parent)
{
	if (!location_level)
		return;
	...
	ajax.success(function(data)
	{
		if (data.success == 1 && !(wplj("#wpl_listing_location_level_container" + location_level).length > 0))
		{
			...
			if (location_level != 'zips')
				wplj("#wpl_listing_location_level_container" + parent_level).after(html);
			else
				wplj("#wpl_listing_all_location_container<?php echo $field->id; ?>").append(html);
		}
