...
{
    ajax = wpl_run_ajax_query('<?php echo wpl_global::get_full_url(); ?>', "wpl_format=b:listing:rooms&wpl_function=delete_room&pid=<?php echo $item_id; ?>&kind=<?php echo $this->kind; ?>&item_id=" + id);
...
function wpl_save_room()
{
    ...
    ajax = wpl_run_ajax_query('<?php echo wpl_global::get_full_url(); ?>', "wpl_format=b:listing:rooms&wpl_function=save_room&pid=<?php echo $item_id; ?>&kind=<?php echo $this->kind; ?>" + "&x_param=" + xroom + "&y_param=" + yroom + "&room_type_id=" + room_type_id + "&room_name=" + room_name);
    ajax.success(function(data)
    {
        added_id = data.data;
...
<div class="new-rooms-wp" id="room_add">
    <select id="room_types<?php echo $field->id; ?>">
        ...
        {
            echo '<option value="'.$room_type['id'].'">'.__($room_type['name'], WPL_TEXTDOMAIN).'</option>';
        }
        ?>
    </select>
